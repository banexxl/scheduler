...
/**
 * Segment Builder Client — Milestone 15.6.1.
 *
 * Interactive rule builder with live count and preview.
 */
...
import { createSegmentAction } from "@/features/segmentation/actions/segment-actions";
...
export default function SegmentBuilderClient({
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const result = await createSegmentAction(tenantSlug, {
        name,
        description: description || undefined,
        rules: buildRuleGroup(),
      });
      if (result.success) {
        router.push(`/${tenantSlug}/customers/segments/${result.segmentId}`);
      } else {
  ...
  return (
    <Stack spacing={3}>
      ...
      <Stack direction="row" spacing={1.5}>
        ...
        <Button variant="outlined" href={`/${tenantSlug}/customers/segments`} size="small">
          Cancel
        </Button>
      </Stack>
    </Stack>
