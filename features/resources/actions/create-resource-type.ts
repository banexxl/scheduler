...
export type ResourceActionResult = { success: boolean; message?: string; fieldErrors?: Record<string, string> };

export async function createResourceTypeAction(tenantSlug: string, values: Record<string, unknown>): Promise<ResourceActionResult> {
  ...
  if (!membership || !["owner", "admin"].includes(membership.role)) return { success: false, message: "Only owners and admins can create resource types." };
  ...
  const { error: insertError } = await supabase.from("resource_types").insert({
    tenant_id: tenant.id,
  ...
  if (insertError) {
    ...
    console.error("[create-resource-type]", insertError.code, insertError.message);
    return { success: false, message: "Unable to create resource type." };
  }

  revalidatePath(`/${tenantSlug}/resources/types`);
  redirect(`/${tenantSlug}/resources/types`);
}
